...
const fetchDataWeather = ({ latitude, longitude, cityName }) => {
  ...
  const currentWeatherUrl = `${BASE_URL}weather?appid=417fcd406debf7f77b7e5aae910974a7${geoCoordinates}`;
  const weatherForecastUrl = `${BASE_URL}forecast?appid=417fcd406debf7f77b7e5aae910974a7${geoCoordinates}`;
  ...
  const weatherForecastData = async () => {
    try {
      ...
      for (let i = 0; i < 5; i++) {
        temperaturesForecast[i] = Number(
          kelvinToC(getDataJson.list[i].main.temp).toFixed(1)
        );
        temperaturesForecastLabels[i] = timeFromTimestamp(
          getDataJson.list[i].dt
        );
      }

      const cloneTemperaturesForecast = [...temperaturesForecast].sort();
      const [maxTemperature, minTemperature] = [
        cloneTemperaturesForecast.pop(),
        cloneTemperaturesForecast.shift(),
      ];
  ...
  return Promise.all([currentWeatherData(), weatherForecastData()]).then(
    (responses) => {
      let weatherData = {
...
export { fetchDataWeather, fetchDataWeatherFake };
